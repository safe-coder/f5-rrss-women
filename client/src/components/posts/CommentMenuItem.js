...
const CommentMenuItem = ({ comment, pos, auth, setonEdit }) => {
  ...
  const MenuItem = () => {
    ...
    return (
      ...
      <div className="commentMenuitemlist">
          <h6 className="commentMenuitemedit" style={{ cursor: 'pointer' }} onClick={handleEditComment}>Edit</h6>
  ...
  return (
    <div className='CommentMenuItem'>
      ...
      {
       menuitem ? ( pos.user._id === auth.user._id ? comment.user._id === auth.user._id ? MenuItem() : <h6 className='commentMenuitemdelete' style={{cursor:'pointer'}} onClick={handleRemove}>Borrar</h6> :comment.user._id === auth.user._id && MenuItem()) : ""
      }
    </div>
  )
}
